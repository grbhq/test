    ...
    def getTurnstileToken(self):
        ...
        while (datetime.now() - start).total_seconds() < timeout:
            try:
                ...
                for _ in range(4):
                    try: 
                        result = self.page.run_js(_EXPAND_JS)
                        logger.info(f"result: {result}")
                    except Exception: pass
                    time.sleep(0.5)
                challengeSolution = self.page.ele("@name=cf-turnstile-response")
                logger.warning(f"challengeSolution: {challengeSolution}")
                # challengeSolution = self.page.wait.eles_loaded('@name=cf-turnstile-response', timeout=10)
